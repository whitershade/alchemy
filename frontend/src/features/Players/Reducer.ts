...
export default handleActions({
    ...
    [cardsTypes.REMOVE_CARD]: (state: any, action: AnyAction) => {
      const { id, playerId, order } = action.payload;
      ...
      return {
        ...
        ...state.data,
            [playerId]: {
          ...state.data[playerId],
              cards: state.data[playerId].cards.reduce(
              (accum:Array<Card>, card:Card) => {
                if (card.id === id) return accum;

                if (card.order > order) {
                  return [...accum, {...card, order: card.order - 1 } ]
                }
